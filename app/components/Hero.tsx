...
const eventStyles = [
  ...
  { icon: CurrencyEur, color: "#06d6a0" },
  { icon: CheckCircle, color: "#06d6a0" },
  { icon: FileText, color: "#8fa3c8" },
...
export function Hero() {
  ...
  const eventTexts = t.raw("events") as { label: string; meta: string }[];
  const events = eventStyles.map((style, i) => ({ ...style, ...eventTexts[i] }));
        ...
        >
          <div className="flex flex-col items-start gap-7">
            ...
            >
              {t("titleLine1")}
              <br />
              <span className="text-mint">{t("titleLine2")}</span>
            </motion.h1>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 border-t border-white/[0.07]">
        <div className="mx-auto grid max-w-5xl grid-cols-3 gap-4 px-6 py-7 text-center">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs text-frost/50">{t("statForderungen")}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xl font-semibold tracking-tight sm:text-3xl">
              <Counter value={98} suffix="%" />
            </span>
            <span className="text-xs text-frost/50">{t("statErfolgsquote")}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xl font-semibold tracking-tight sm:text-3xl">
              <Counter value={0} suffix=" €" />
            </span>
            <span className="text-xs text-frost/50">{t("statVorkosten")}</span>
          </div>
        </div>
      </div>
    </section>
