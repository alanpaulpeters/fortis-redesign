...
const avatars = [
  "/media/avatar-natalie.jpg",
  "/media/avatar-daniel.jpg",
  "/media/avatar-moritz.jpg",
  "/media/avatar-vilja.jpg",
];

export function Trust() {
  const t = useTranslations("trust");
  const testimonials = (t.raw("testimonials") as { name: string; quote: string }[]).map(
    (item, i) => ({ ...item, avatar: avatars[i] })
  );
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            <span className="text-sm text-frost/55">{t("stat1Label")}</span>
          </div>
          <div className="flex flex-col gap-2">
            <span className="text-5xl font-semibold tracking-tight">
              <Counter value={0} suffix=" €" />
            </span>
            <span className="text-sm text-frost/55">{t("stat2Label")}</span>
          </div>
          <div className="flex flex-col items-start gap-3">
            ...
            <span className="text-sm text-frost/55">{t("schufaLabel")}</span>
          </div>
        </motion.div>
            ...
            >
              ...
              <figcaption className="flex items-center gap-3">
                ...
                <span className="flex flex-col">
                  <span className="text-[13px] font-medium text-frost/90">
                    {testimonial.name}
                  </span>
                  <span className="text-[11.5px] text-mint" aria-label={t("starsLabel")}>
                    ★★★★★
                  </span>
                </span>
              </figcaption>
